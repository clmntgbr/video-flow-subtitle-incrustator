...
from src.Protobuf.Message_pb2 import ApiToSubtitleIncrustator, MediaPodStatus, MediaPod
...
def process_message(message):
    ...
    try:
        ...
        protobuf.mediaPod.status = MediaPodStatus.Name(
            MediaPodStatus.SUBTITLE_INCRUSTATOR_COMPLETE
        )
        ...
        protobuf.mediaPod.status = MediaPodStatus.Name(
            MediaPodStatus.SUBTITLE_INCRUSTATOR_ERROR
        )
        if not rmq_client.send_message(
            protobuf, "App\\Protobuf\\SubtitleIncrustatorToApi"
        ):
            return False
